...
export class SprintsService {
  ...
  getAllEmployeesNoExistOnTeamBySprintId(sprintId:string | null):Observable<any>{
    return this.httpClient.get('http://localhost:8020/employee/employeenoexistonsprint/'+sprintId);
  }
  getAllEmployeesExistOnTeamBySprintId(sprintId:string | null):Observable<any>{
    return this.httpClient.get('http://localhost:8020/employee/employeeexistonsprint/'+sprintId);
  }
  saveScoreSprintFinal(sprintId:string | null,sprintScoreFinal:any):Observable<any>{
    return this.httpClient.put('http://localhost:8020/sprintday/update-score/'+sprintId,sprintScoreFinal);
  }
  getAllEmployeeSprint(sprintEmployeeId:string | null):Observable<any>{
    return this.httpClient.get(this.API_SERVER+'/sprint-employee/id/'+sprintEmployeeId);
  }
  updateEmployeeSprint(sprintEmployeeId:string | null,sprintEmployee:any):Observable<any>{
    return this.httpClient.put(this.API_SERVER+'/sprint-employee/update/'+sprintEmployeeId,sprintEmployee);
  }
  ...
  getAllDatesSprint(sprintId:string | null):Observable<any>{
    return this.httpClient.get('http://localhost:8020/sprintday/sprint/'+sprintId);
  }
}
